...
export default function PolygonDrawingApp() {
  ...
  const handleCanvasClick = useCallback(
    (point: Point) => {
      ...
      if (!newState.activePolygon) {
        ...
        if (activePolygon.points.length >= 3 && isNearPoint(point, activePolygon.points[0], 15)) {
          activePolygon.completed = true
          activePolygon.label = currentLabel || `Polygon ${newState.polygons.length + 1}`
  ...
  const handleSavePolygon = useCallback(() => {
    if (!appState.activePolygon || appState.activePolygon.points.length < 3) return
  ...
  const handleDeletePolygon = useCallback(
    (id: string) => {
      const newState = {
        ...
        selectedPolygonId: appState.selectedPolygonId === id ? null : appState.selectedPolygonId,
      }
      saveToHistory(newState)
    },
    ...
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">Polygon Drawing App</h1>
          <div className="text-sm text-gray-500">
            {appState.backgroundImage
              ? "Click to place points on image • Click near start to close polygon"
              : "Upload an image or click to place points • Click near start to close polygon"}
          </div>
        </div>
      </header>
